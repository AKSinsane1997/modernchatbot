...
import chatbot1 from "./assets/chatbot.gif";
import bot from "./assets/bot.png";
import header from "./assets/bot_header.png";
import "./App.css";
...
const handleApiCall = async (url, body) => {
  try {
    ...
    let message = response.data.output.text.trim(); // Ensure message is trimmed

    if (
      message === "The list of schemes that you are eligible is as follows:\n-"
    ) {
      return "There are no schemes available for you right now.";
    }

    if (
      message ===
      "This Feature if for scheme eligibility only. Please input you age, gender, occupation, marital status, family income, disability status and community to know your eligible schemes.\n                    "
    ) {
      return "Please input your age, gender, occupation, marital status, family income, disability status, and community to know your eligible schemes.";
    }
    ...
    if (url === chatUrl) {
      message = message.replace(/Source:.*$/, ""); // Clean up any source information
    }
  ...
  const flow = {
    ...
    handle_choose: {
      ...
      path: (params) => {
        ...
        if (selectedOption === t("menu")) {
          return "second_loop";
        } else if (selectedOption === t("lang")) {
    ...
    prompt_user_query: {
      ...
      options: [t("yes"), t("no")],
      path: (params) => {
        if (params.userInput === t("yes")) {
          return "next_question_query";
        } else if (params.userInput === t("no")) {
          return "third_loop";
        }
      },
    },
